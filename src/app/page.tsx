      ...
      <div className="relative z-10">
            ...
            <div className="grid grid-cols-1 gap-4 pt-12">
              {mobileSkills.map((skill, i) => (
                <div key={i} className="rounded-xl border border-line bg-surface p-5">
                  <h3 className="text-xl font-semibold text-ink mb-4">{skill.title}</h3>
                  <div className="flex flex-wrap gap-2">
                    {skill.tags.map((tag, j) => (
                      <span key={j} className="font-mono text-sm uppercase tracking-wider px-3 py-1.5 rounded-full border border-line bg-surface2/70 text-muted">
                        {tag}
                      </span>
                    ))}
                  </div>
        ...
        <footer className="border-t border-line">
          <div className="mx-auto flex max-w-[1600px] flex-col items-center justify-between gap-4 px-5 py-8 sm:flex-row md:px-10">
            ...
            <div className="flex items-center gap-6 font-mono text-sm uppercase tracking-[0.2em]">
              <a href="https://github.com/tarun-ss" target="_blank" rel="noopener noreferrer" className="link-line text-muted transition-colors hover:text-accent">GitHub</a>
              <a href="https://linkedin.com/in/tarun-s-192273223" target="_blank" rel="noopener noreferrer" className="link-line text-muted transition-colors hover:text-accent">LinkedIn</a>
              <a href="mailto:tarun.sathya23@gmail.com" className="link-line text-muted transition-colors hover:text-accent">Email</a>
            </div>
          </div>
        </footer>
      </div>
