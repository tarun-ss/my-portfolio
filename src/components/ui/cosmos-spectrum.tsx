...
export function CosmicSpectrum({ color = "original", blur = false, title = "Cosmos Spectrum", subtitle = "Where Design Becomes Communication\nAcross the World" }: CosmicSpectrumProps) {
  ...
  const splitText = (text: string, className = "") => {
    return text.split("").map((char, index) => (
      <span key={index} className={`char ${className} inline-block`}>
        {char === " " ? "\u00A0" : char}
      </span>
    ))
  }

  const currentColors = colorThemes[color as keyof typeof colorThemes] || colorThemes.original
  ...
  return (
    <div ref={containerRef} className="relative min-h-screen overflow-x-hidden bg-black text-white">
      ...
      {/* Hero Section */}
      <section className="h-screen w-full p-8 flex flex-col justify-center relative z-20">
        <h1 className="hero-title text-center text-6xl md:text-7xl font-bold tracking-tighter transition-colors duration-300">
          {splitText(title)}
        </h1>
      </section>

      <div className="nav-bottom-center fixed bottom-8 left-1/2 transform -translate-x-1/2 z-[1000] pointer-events-auto text-sm uppercase tracking-wide transition-colors duration-300 scroll-hint">
      ...
      <div className="animation-section h-screen relative">
        <div className="absolute bottom-0 left-0 right-0 h-screen pointer-events-none z-10">
          {/* SVG Container */}
          <div
          ...
          {/* Main Title */}
          <div
            className="main-title absolute bottom-1/2 left-1/2 transform translate-x-[-50%] translate-y-1/2 text-center text-xl md:text-2xl leading-relaxed z-20 opacity-0 transition-colors duration-300 font-medium"
